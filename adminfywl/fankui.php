...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'fankui where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','fankui.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."fankui where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='fankui.php';</script>";
}
}
					?>
<!DOCTYPE html>
...
<br>
				...
				<div class="panel panel-default">
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
								...
								<th width="180" class="sort">反馈人</th>
								<th width="180" class="sort"><span>反馈内容</span></th>
								<th class="sort" width="95">反馈时间</th>
								<th class="sort" width="80">回复内容</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'fankui where remark like "%'.$_GET['key'].'%"   order by ID desc , id desc';
 							}
						 
							else{
									$sql = 'select * from '.flag.'fankui  order by ID desc , id desc';
 								}
						 ...
						 $remark=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['remark']);
 							...
 							?>
									...
									<td align=""><?=get_user('username',$row['uid'])?></td>
									<td align="">
									 <?=$remark?>								</td>
									<td><?=$row['date']?></td>
									<td><?=$row['huifu']?></td>
									...
									<td>
                                         ...
                                         <a class="btn" href="fankui_edit.php?id=<? echo $row['ID']?>" >回复</a>
