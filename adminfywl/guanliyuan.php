...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'guanliyuan where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','guanliyuan.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."guanliyuan where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='guanliyuan.php';</script>";
}
}
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<a href="guanliyuan_add.php" class="btn btn-primary">新增</a>
					<a href="/guanliyuan" class="btn btn-primary">管理员地址</a>
...
//-->
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
						   	<th   width="20"><span onClick="selectBox('reverse')">选</span></th>
								<th width="180" class="sort"><span>权限</span></th>
								<th class="sort" width="95">登录账号</th>
								<th class="sort" width="80">登录密码</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'guanliyuan where loginname like "%'.$_GET['key'].'%"   order by ID desc , id desc';
 							}
							else{
									$sql = 'select * from '.flag.'guanliyuan  order by ID desc , id desc';
 								}
						 ...
						 $key1=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['i_name']);
 							...
 							?>
									...
									<td>
                                         ...
                                         <a class="btn" href="guanliyuan_edit.php?id=<? echo $row['ID']?>" >编辑</a>
