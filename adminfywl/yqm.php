...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'yqm where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','yqm.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."yqm where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='yqm.php';</script>";
}
}
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<a href="yqm_add.php" class="btn btn-primary">新增</a>
...
//-->
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
						  	<th   width="20"><span onClick="selectBox('reverse')">选</span></th>
								<th width="180" class="sort">状态</th>
								<th class="sort" width="95">邀请码</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'yqm where card like "%'.$_GET['key'].'%"   order by ID desc , id desc';
 							}
							else{
									$sql = 'select * from '.flag.'yqm  order by ID desc , id desc';
 								}
						 ...
						 {$zt='使用者['.get_user('username',$row['uid']).']';}
