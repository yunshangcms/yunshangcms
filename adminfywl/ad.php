...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'ad where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','ad.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."ad where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='ad.php';</script>";
}
}
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<a href="ad_add.php" class="btn btn-primary">新增</a>
...
//-->
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
						        ...
						        <th data-field="name">广告链接</th>
						        <th data-field="name">广告图</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'ad where a_name like "%'.$_GET['key'].'%"   order by id desc , id desc';
 							}
						 
							else{
									$sql = 'select * from '.flag.'ad  order by id desc , id desc';
 								}
						 ...
						 $key1=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['a_name']);
 							...
 							?>
									...
									<td align=""><?=$a_name?></td>
									<td> <a href="<? echo $row['a_url']?>"  target="_blank">	点击查看	</a></td>
									<td> <a href="<? echo $row['a_pic']?>"  target="_blank">	点击查看	</a></td>
									  
									<td>
								 
                                         <a class="btn" href="ad_edit.php?id=<? echo $row['ID']?>" >编辑</a>
