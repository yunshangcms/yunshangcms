...
if (isset($_POST['提交'])){
	...
	$sql = 'update '.flag.'shipin set '.arrtoupdate($_data).' where id = '.$_GET['id'].'';
	if (mysql_query($sql)) {
		
		mysql_query('update '.flag.'usershipin set cid = '.$_POST['i_cid'].' where vid = '.$_GET['id'].'');
		alert_href('修改成功!', 'detail.php');
	} else {
		alert_back('修改失败!');
	}
}
 ...
 <br>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading"><span class="glyphicon glyphicon-envelope"></span> 修改视频</div>
				...
				<form method="post" name="formrec"   class="form-horizontal"  id="formrec" role="form">
							<fieldset>
								...
								<div class="form-group">
									...
									<div class="col-md-9">
																<input  class="form-control"  placeholder="外链请直接输入图片地址" type="text" name="i_pic" id="i_pic"   value="<?=$row['image']?>"> 
  ...
  <div class="layui-input-inline w450">
					 	...
					 	<div class="form-group">
									...
									<?php
						$result1 = mysql_query('select * from '.flag.'channel    order by sort desc ,id desc');
						while($row1 = mysql_fetch_array($result1)){
						?>
			                <option  <? if ($row['cid'] == $row1['ID']) { echo "selected" ;}?> value="<? echo $row1['ID'];?>"><? echo $row1['name'];?></option>
					 	...
					 	<div class="form-group">
									...
									<div class="col-md-9">
			                <input name="i_name" type="text"  class="form-control" id="c_name"   placeholder="视频名称"    value="<?=$row['name']?>" >
					 	...
					 	<div class="form-group">
									...
									<div class="col-md-9">
		  <input name="i_order" lay-verify="number" autocomplete="off"  value="<?=$row['sort']?>" placeholder="输入顺序"  class="form-control" type="text">
...
.ke-button-common   { height:40PX; margin-top:10px}
							...
							<input id="url"   class="form-control" type="text" name="i_durl1"  placeholder="外链请直接输入视频地址"  value="<?=$row['url']?>" >
