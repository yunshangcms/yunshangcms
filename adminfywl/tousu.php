...
if($_GET['act'] =='del'){
	$sql = 'delete from '.flag.'tousu where id = '.$_GET['id'].'';
	if(mysql_query($sql)){
		alert_href('删除成功!','tousu.php');
	}else{
		alert_back('删除失败！');
	}
}
...
/*如果要获取全部数值则使用下面代码*/
   $id= implode(",",$_POST['id']);
   $str="DELETE FROM ".flag."tousu where id in ($id)";
   mysql_query($str);
  echo "<script>alert('删除成功！');window.location.href='tousu.php';</script>";
}
}
		...
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading">投诉列表</div>
...
//-->
					...
					<table  class="table table-hover">
						    ...
						    <tr align="center">
						        ...
						        <th data-field="name">投诉IP</th>
						        <th data-field="name">投诉分类</th>
						        <th data-field="name">投诉内容</th>
						        <th data-field="name">投诉状态</th>
  ...
  if (isset($_GET['key'])) {
 $sql = 'select * from '.flag.'tousu where ip like "%'.$_GET['key'].'%"   order by ID desc , id desc';
 							}
						 
							else{
									$sql = 'select * from '.flag.'tousu  order by ID desc , id desc';
 								}
						 ...
						 $ip=str_replace($_GET['key'],"<font color=red> ".$_GET['key']."</font>",$row['ip']);
